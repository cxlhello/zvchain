...
func (api *RpcGtasImpl) Balance(account string) (*Result, error) {
	...
	b := core.BlockChainImpl.GetBalance(addr)
...
func (api *RpcGtasImpl) MinerInfo(addr string, detail string) (*Result, error) {
	...
	if detail != "" && detail != "all" && !common.ValidateAddress(detail) {
	...
	default:
		detailAddr, _ := common.StringToAddress(detail)
		details := core.MinerManagerImpl.GetStakeDetails(address, detailAddr)
		m := make(map[string][]*StakeDetail)
...
func (api *RpcGtasImpl) ViewAccount(hash string) (*Result, error) {
	address, err := common.StringToAddress(hash)
	if err != nil {
	...
	if len(account.Code) > 0 {
		...
		iter := accountDb.DataIterator(address, []byte{})
		for iter.Next() {
			k := string(iter.Key[:])
			v := string(iter.Value[:])
